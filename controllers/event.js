...
exports.getOneBilletterie = (req, res, next) =>{
    funcs.bddQuery(req.conBDA, "SELECT * FROM newBilletterie WHERE event_id = ?", [req.body.id])
    .then((data) => {
        if (data == undefined || data.length <1) {
...
exports.createBilletterie = (req, res, next) => {
    ...
    if (body.title && body.description && body.dateEvent && body.event_place && body.thumbnail && body.pole_id && body.loginSender && body.date_open && body.date_close && body.num_places && body.cost_contributor && body.cost_non_contributor && body.points) {
        ...
        funcs.bddQuery(req.conBDA, "INSERT INTO `newBilletterie` (`event_id`, `title`, `description`, `dateEvent`, `event_place`, `pole_id`, `login_creator`, `date_open`, `date_close`, `num_places`, `cost_contributor`, `cost_non_contributor`, `points`, `on_sale`, `thumbnail`) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [body.title , body.description , body.dateEvent , body.event_place , body.pole_id , body.loginSender , body.date_open , body.date_close , body.num_places , body.cost_contributor,  body.cost_non_contributor, body.points, 0 /* Billetterie fermée lors de sa création*/, body.thumbnail])
    ...
    .then((data) => {
        ...
        if (body.sendMail) {
            ...
            .then((data)=> {
                ...
                funcs.bddQuery(req.conBDA, "SELECT newUsers.email FROM newUsers JOIN newBilletterie ON newUsers.login = newBilletterie.login_creator WHERE newBilletterie.event_id = ?", [req.body.event_id])
                .then((data) => {
