...
exports.createBilletterie = (req, res, next) => {
    const body = req.body;
    if (body.title && body.description && body.dateEvent && body.event_place && body.pole_id && body.loginSender && body.date_open && body.date_close && body.num_places && body.cost_contributor && body.const_non_contributor && body.points) {
        funcs.bddQuery(req.conBDA, "INSERT INTO newBilleterie ('title', 'description', 'dateEvent', 'event_place', 'pole_id', 'login_creator', 'date_open', 'date_close', 'num_places', 'cost_contributor', 'const_non_contributor', 'points', 'on_sale') VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [body.title , body.description , body.dateEvent , body.event_place , body.pole_id , body.loginSender , body.date_open , body.date_close , body.num_places , body.cost_contributor,  body.const_non_contributor, body.points, 0 /* billeterie fermée lors de sa création*/])
...
exports.modifyBilletterie = (req, res, next) => {
    ...
    funcs.bddQuery(req.conBDA, "SELECT * FROM newBilleterie WHERE event_id = ?", [body.event_id])
    .then((data) => {
        if (data == undefined || data.length < 1){
...
exports.deleteBilletterie = (req, res, next) => {
    ...
    .then(() => {
        ...
        funcs.bddQuery(req.conBDA, "DELETE FROM newBilleterie WHERE event_id = ?", [body.event_id])
